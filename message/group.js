...
module.exports = welcome = async (dha, anu) => {
		...
		try {
            ...
            Haikal.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik ${prefix}menu', 'conversation')
            }
             if (anu.action == 'add' && !mem.includes(Haikal.user.jid)) {
             if (!welkom.includes(anu.jid)) return
                mdata = await Haikal.groupMetadata(anu.jid)
                ...
                imageMsg = (await Haikal.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${teks}`, footerText: 'JANGAN BUAT ONAR BRO', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await Haikal.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                Haikal.relayWAMessage(prep)
}
            if (anu.action == 'remove' && !mem.includes(Haikal.user.jid)) {
            if (!welkom.includes(anu.jid)) return
                mdata = await Haikal.groupMetadata(anu.jid)
            	num = anu.participants[0]
                let w = Haikal.contacts[num] || { notify: num.replace(/@.+/, '') }
